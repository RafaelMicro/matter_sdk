...
void AppTask::InitServer(intptr_t arg)
{
    ...
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(NotSpecified, "ContMgr::Init() failed");
    }
...
void AppTask::ChipEventHandler(const ChipDeviceEvent * aEvent, intptr_t /* arg */)
{
    ...
    case DeviceEventType::kServerReady:
        if (sCommissioned)
        {
            ...
            if (sOperationalResumeTimingValid)
            {
                ...
                ChipLogProgress(NotSpecified,
                                ...
                                static_cast<uint64_t>(resumeDuration.count()));
        ...
        if (sBootToServerReadyTimingValid)
        {
            ...
            ChipLogProgress(NotSpecified,
                            ...
                            static_cast<uint64_t>(sBootToServerReadyStartTimestamp.count()),
                            static_cast<uint64_t>(now.count()),
    ...
    case DeviceEventType::kCommissioningComplete:
        ...
        if (sCommissioningStartValid)
        {
            ...
            const auto paseDuration = sPaseTimingValid
                ? (sPaseEndTimestamp - sCommissioningStartTimestamp)
                : chip::System::Clock::Milliseconds64(0);
            const auto caseDuration = sCaseTimingValid
                ? (sCaseEndTimestamp - sCaseStartTimestamp)
                : chip::System::Clock::Milliseconds64(0);
            ...
            ChipLogProgress(NotSpecified,
                            ...
                            static_cast<uint64_t>(sCommissioningStartTimestamp.count()),
                            static_cast<uint64_t>(completion.count()),
        ...
        else if (sessionType == chip::Transport::SecureSession::Type::kCASE)
        {
            if (sCaseStageInProgress)
            {
                ...
                ChipLogProgress(NotSpecified,
                                ...
                                static_cast<uint64_t>(sCaseStartTimestamp.count()),
                                static_cast<uint64_t>(now.count()),
                                static_cast<uint64_t>(caseDuration.count()));
            }
...
void AppTask::FunctionHandler(AppEvent * aEvent)
{
    ...
    case AppEvent::AppActionTypes::kActionTypes_Switch_1:
        if (aEvent->ButtonEvent.Action == true)
        {
            ContMgr().ToggleStateValue(false);
        }
        else
        {
            ContMgr().ToggleStateValue(true);
        }
        break;
    default:
        break;
