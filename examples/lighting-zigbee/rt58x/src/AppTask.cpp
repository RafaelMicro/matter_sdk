...
void AppTask::InitServer(intptr_t arg)
{
    ...
    else
    {
        ...
        {
            /*After OTA from single protocol to multi protocol*/
            GetMultiApp().SuspendZigbeeTask();
            GetMultiApp().SetCurrentProtocol(MultiApp::CurrentProtocol::kMatter);
        }
...
void AppTask::ChipEventHandler(const ChipDeviceEvent * aEvent, intptr_t /* arg */)
{
    ...
    case DeviceEventType::PublicPlatformSpecificEventTypes::kBleToApp:
        log_info("[Matter] Receive BLE message:");
        log_info_hexdump("", aEvent->Platform.TRSPData.data, aEvent->Platform.TRSPData.len);
        BLEMgrImpl().BLESendMessage(aEvent->Platform.TRSPData.data, aEvent->Platform.TRSPData.len);
        break;
#endif
#if APP_ENABLE_COMMISSIONING_TIMING
