...
HOSAL_UART_DEV_DECL(app_uart_dev, 1,
                    28,
                    29, UART_BAUDRATE_115200)
...
void AppTask::UpdateThermoStatUI()
{
    ...
    ChipLogProgress(NotSpecified, "Thermostat Status - M:%d T:%d'C H:%d'C C:%d'C", TempMgr().GetMode(), TempMgr().GetCurrentTemp(),
    ...
    hosal_uart_send(&app_uart_dev, uart_report_data, 15);
    log_hexdump_out("Tx", 16, uart_report_data, 15);
}
