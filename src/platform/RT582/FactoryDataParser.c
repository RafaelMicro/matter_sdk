...
uint16_t * states; // this value will be setting in #include <logging/log.h>

#define MAX_FACTORY_DATA_NESTING_LEVEL 3

static inline bool uint16_decode(uint16_t * states, uint16_t * value)
{
    // uint32_t u32;
...
bool ParseFactoryData(uint8_t * buffer, uint16_t bufferSize, struct FactoryData * factoryData)
{
    ...
    // struct rtcbor_string currentString;
    ...
    //                 1000 * (date.value[0] - '0') + 100 * (date.value[1] - '0') + 10 * (date.value[2] - '0') + date.value[3] -
    //                 '0';
    ...
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->hw_ver_str);
    //     }
    //     else if (strncmp("rd_uid", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->rd_uid);
    //     }
    //     else if (strncmp("dac_cert", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->dac_cert);
    //     }
    //     else if (strncmp("dac_key", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->dac_priv_key);
    //     }
    //     else if (strncmp("pai_cert", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->pai_cert);
    //     }
    //     else if (strncmp("spake2_salt", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->spake2_salt);
    //     }
    //     else if (strncmp("spake2_verifier", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->spake2_verifier);
    //     }
    //     else if (strncmp("vendor_name", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->vendor_name);
    //     }
    //     else if (strncmp("product_name", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->product_name);
    //     }
    //     else if (strncmp("enable_key", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->enable_key);
    //     }
    //     else if (strncmp("user", (const char *) currentString.value, currentString.len) == 0)
    //     {
    //         res = res && zcbor_bstr_decode(states, (struct rtcbor_string *) &factoryData->user);
    //     }
    //     else
    //     {
