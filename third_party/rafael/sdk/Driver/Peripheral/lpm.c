...
void Lpm_Set_Platform_Low_Power_Wakeup(low_power_platform_enter_mode_cfg_t platform_low_power_mode)
{
    do
    {
        ...
        if (platform_low_power_mode == LOW_POWER_PLATFORM_ENTER_SLEEP)
        {
            ...
            if ((low_power_wakeup & LOW_POWER_WAKEUP_RTC_TIMER) || (low_power_wakeup & LOW_POWER_WAKEUP_COMPARATOR) ||
                (low_power_wakeup & LOW_POWER_WAKEUP_32K_TIMER) || (low_power_wakeup_gpio != 0))
            {
        ...
        else if (platform_low_power_mode == LOW_POWER_PLATFORM_ENTER_DEEP_SLEEP)
        {
            ...
            {
                SYSCTRL->SYS_CLK_CTRL |=
                    HCLK_SLEEP_RUN_IN_32K;                 /* HCLK sleep selection in Sleep, HCLK is forced to 32KHz in Sleep */
            ...
            set_deepsleep_wakeup_pin(
                low_power_wakeup_gpio); /* Set the corresponding bits to enable the wakeup of GPIOx in Deep Sleep */

            if (low_power_wakeup & LOW_POWER_WAKEUP_COMPARATOR)
            {
                PMU->PMU_COMP0.bit.AUX_COMP_EN_DS =
                    1; /* AUX comparator enable selection in Sleep, AUX comparator enable in DeepSleep */
                PMU->PMU_COMP0.bit.AUX_COMP_DS_WAKEUP_EN =
                    1; /* AUX comparator wakeup enable selection in Sleep, AUX comparator wakeup enable in DeepSleep */
            }
            else
            {
                PMU->PMU_COMP0.bit.AUX_COMP_EN_DS =
                    0; /* AUX comparator enable selection in Sleep, AUX comparator disable in DeepSleep */
                PMU->PMU_COMP0.bit.AUX_COMP_DS_WAKEUP_EN =
                    0; /* AUX comparator wakeup enable selection in Sleep, AUX comparator wakeup disable in DeepSleep */
            }
        }
...
{
    set_deepsleep_wakeup_invert(
        value); /* Set the corresponding bit0~bit31 to invert the GPIO0~GPIO31 for wakeup in DeepSleep Mode */
}
...
{
    set_sram_shutdown_normal(
        value); /* SRAM shut-down control in Normal Mode, set the corresponding bit0~bit4 to shut-down SRAM0~SRAM4 in Normal Mode */
}

void Lpm_Set_Sram_Sleep_Deepsleep_Shutdown(uint32_t value)
{
    set_sram_shutdown_sleep(value); /* SRAM shut-down control in Low Power Mode, set the corresponding bit0~bit4 to shut-down
                                       SRAM0~SRAM4 in Sleep/DeepSleep Mode */
}
    ...
    if (low_power_mask_status == LOW_POWER_NO_MASK)
    {
        ...
        if (low_power_level == LOW_POWER_LEVEL_SLEEP3)
        {
            ...
            COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_HOST |=
                COMMUMICATION_SUBSYSTEM_DEEPSLEEP; /* communication system enter deep sleep mode */
            Lpm_Comm_Subsystem_Check_System_DeepSleep();
            Lpm_Set_Sram_Sleep_Deepsleep_Shutdown(LPM_SRAM0_RETAIN);
            Lpm_Set_Platform_Low_Power_Wakeup(
                LOW_POWER_PLATFORM_ENTER_DEEP_SLEEP); /* set platform system wakeup source when entering deep sleep mode*/
        ...
        else if (low_power_level == LOW_POWER_LEVEL_SLEEP2)
        {
            ...
            Lpm_Set_Platform_Low_Power_Wakeup(
                LOW_POWER_PLATFORM_ENTER_SLEEP); /* set platform system wakeup source when entering sleep mode*/
            ...
            if (comm_subsystem_wakeup_mask_status != COMM_SUBSYS_WAKEUP_NO_MASK)
            {
                COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_HOST |= COMMUMICATION_SUBSYSTEM_HOSTMODE; /* enable host mode */
                COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_HOST |=
                    COMMUMICATION_SUBSYSTEM_WAKEUP; /* communication system wakeup from deep sleep mode */
        ...
        else if (low_power_level == LOW_POWER_LEVEL_SLEEP1)
        {
            ...
            if (comm_subsystem_wakeup_mask_status != COMM_SUBSYS_WAKEUP_NO_MASK)
            {
                COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_HOST |= COMMUMICATION_SUBSYSTEM_HOSTMODE; /* enable host mode */
                COMM_SUBSYSTEM_AHB->COMM_SUBSYSTEM_HOST |=
                    COMMUMICATION_SUBSYSTEM_WAKEUP; /* communication system wakeup from sleep mode */
            }
        }
