...
static EfErrCode read_env(env_node_obj_t env)
{
    ...
    if (env->len == ~0UL || env->len > ENV_AREA_SIZE || env->len < ENV_NAME_LEN_OFFSET)
    {
        ...
        if (env->status != ENV_ERR_HDR)
        {
            ...
            // info("Error: The ENV @0x%08X length has an error.\n", env->addr.start);
        ...
        else if (sector->status.store == SECTOR_STORE_USING)
        {
            ...
            while ((env_meta.addr.start = get_next_env_addr(sector, &env_meta)) != FAILED_ADDR)
            {
                ...
                if (!env_meta.crc_is_ok)
                {
                    ...
                    {
                        // info("Error: The ENV (@0x%08X) CRC32 check failed!\n", env_meta.addr.start);
                        sector->remain = 0;
                        result         = EFD_READ_ERR;
                        break;
                    }
...
static bool find_env_cb(env_node_obj_t env, void * arg1, void * arg2)
{
    const char * key = arg1;
    bool * find_ok   = arg2;
    size_t key_len   = utility_strlen(key);
...
char * efd_get_env(const char * key)
{
    ...
    if ((get_size = efd_get_env_blob(key, value, EFD_STR_ENV_VALUE_MAX_SIZE, NULL)) > 0)
    {
        ...
        {
            // info("Warning: The ENV value isn't string. Could not be returned\n");
            return NULL;
        }
    }
...
static uint32_t alloc_env(sector_meta_data_t sector, size_t env_size)
{
    ...
    if (empty_sector > 0 && empty_env == FAILED_ADDR)
    {
        ...
        else
        {
            ...
            // info("Trigger a GC check after alloc ENV failed.\n");
            gc_request = true;
        }
    }
    ...
    else
    {
        ...
        if (!last_is_complete_del && result == EFD_NO_ERR)
        {
            ...
            if (key != NULL)
            {
                ...
                update_env_cache(key, utility_strlen(key), FAILED_ADDR);
            }
...
__retry:
    ...
    {
        // info("Warning: Alloc an ENV (size %d) failed when new ENV. Now will GC then retry.\n", env_size);
        gc_collect();
        already_gc = true;
        goto __retry;
    }
    ...
    if (sector->check_ok && (sector->status.dirty == SECTOR_DIRTY_TRUE || sector->status.dirty == SECTOR_DIRTY_GC))
    {
        ...
        while ((env.addr.start = get_next_env_addr(sector, &env)) != FAILED_ADDR)
        {
            ...
            if (env.crc_is_ok && (env.status == ENV_WRITE || env.status == ENV_PRE_DELETE))
            {
                ...
                {
                    // info("Error: Moved the ENV (%.*s) for GC failed.\n", env.name_len, env.name);
                }
            }
        }
        format_sector(sector->addr, SECTOR_NOT_COMBINED);
        // info("Collect a sector @0x%08X\n", sector->addr);
    }
...
static EfErrCode create_env_blob(sector_meta_data_t sector, const char * key, const void * value, size_t len)
{
    ...
    if (utility_strlen(key) > EFD_ENV_NAME_MAX)
    {
        // info("Error: The ENV name length is more than %d\n", EFD_ENV_NAME_MAX);
        return EFD_ENV_NAME_ERR;
    }

    memset(&env_hdr, 0xFF, sizeof(struct env_hdr_data));
    env_hdr.magic     = ENV_MAGIC_WORD;
    env_hdr.name_len  = utility_strlen(key);
    ...
    {
        // info("Error: The ENV size is too big\n");
        return EFD_ENV_FULL;
    }
    ...
    if (env_addr != FAILED_ADDR || (env_addr = new_env(sector, env_hdr.len)) != FAILED_ADDR)
    {
        ...
        {
            // info("Trigger a GC check after created ENV.\n");
            gc_request = true;
        }
    }
    else
...
EfErrCode efd_del_env(const char * key)
{
    ...
    if (!init_ok)
    {
        // info("Error: ENV isn't initialize OK.\n");
        return EFD_ENV_INIT_FAILED;
    }
...
static EfErrCode set_env(const char * key, const void * value_buf, size_t buf_len)
{
    ...
    else
    {
        ...
        if (new_env_by_kv(&sector, utility_strlen(key), buf_len) == FAILED_ADDR)
        {
            return EFD_ENV_FULL;
        }
...
{
    return efd_set_env_blob(key, value, utility_strlen(value));
}
...
EfErrCode efd_env_set_default(void)
{
    ...
    for (i = 0; i < default_env_set_size; i++)
    {
        ...
        {
            value_len = utility_strlen(default_env_set[i].value);
        }
        else
        {
    ...
    if (env->crc_is_ok)
    {
        ...
        *using_size += env->len;
        ...
        if (env->status == ENV_WRITE)
        {
            ...
            {
                info("blob @0x%08X %dbytes", env->addr.value, env->value_len);
            }
            info("\n");
        }
    }
...
static void env_auto_update(void)
{
    ...
    if (get_env(VER_NUM_ENV_NAME, &saved_ver_num, sizeof(size_t), NULL) > 0)
    {
        ...
        if (saved_ver_num != setting_ver_num)
        {
            ...
            size_t i, value_len;
            struct sector_meta_data sector;
            // info("Update the ENV from version %d to %d.\n", saved_ver_num, setting_ver_num);
            for (i = 0; i < default_env_set_size; i++)
            {
...
static bool check_sec_hdr_cb(sector_meta_data_t sector, void * arg1, void * arg2)
{
    if (!sector->check_ok)
    {
        ...
        // info("Warning: Sector header check failed. Format this sector (0x%08x).\n", sector->addr);
        (*failed_count)++;
        format_sector(sector->addr, SECTOR_NOT_COMBINED);
    }
...
static bool check_and_recovery_env_cb(env_node_obj_t env, void * arg1, void * arg2)
{
    ...
    {
        // info("Found an ENV (%.*s) which has changed value failed. Now will recovery it.\n", env->name_len, env->name);
        /* recovery the old ENV */
        if (move_env(env) == EFD_NO_ERR)
        {
            // info("Recovery the ENV successful.\n");
        }
        else
        {
            // info("Warning: Moved an ENV (size %d) failed when recovery. Now will GC then retry.\n", env->len);
            return true;
        }
    }
...
EfErrCode efd_load_env(void)
{
    ...
    {
        // info("Warning: All sector header check failed. Set it to default.\n");
        efd_env_set_default();
    }
...
EfErrCode efd_env_init(efd_env const * default_env, size_t default_env_size)
{
    ...
    // info("ENV start address is 0x%08X, size is %d bytes.\n", EFD_START_ADDR, ENV_AREA_SIZE);
